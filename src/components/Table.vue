    ...
    <table class="table">
      ...
      <tr>
        <th v-for="(header, i) in headers" :key="`${header}-${i}`">
          <Sorting v-if="header !== 'url'" :header="header" :direction="getDirection(header)" @sorting="handleSorting"/>
      ...
      <tr v-for="(data, i) in sortedItems" :key="i">
        ...
        <td :data-label="'URL'"><a :href="data.url" target="_blank">{{ data.url }}</a></td>
      </tr>
      </tbody>
    </table>
  </div>
...
const filteredItems = computed(() => {
  if (searchFilter.value !== '') {
    ...
    return props.data.results?.filter((item) =>
